...
** backend
:PROPERTIES:
:header-args: :tangle ./digraph.el :emacs-lisp :lexical t
:END:
...
#+RESULTS:
: digraphel-backend-adapt

#+header: :noweb-ref el-example
#+begin_src emacs-lisp
  (digraphel-backend-adapt
   :adaptee-suffix "value-as-string"
   :target-suffix "vertex-as-string"
   :key-map '(:vertex :value))
#+end_src

#+RESULTS:
: digraphel-vertex-as-string

#+begin_src emacs-lisp
  (digraphel-vertex-as-string :vertex "X")
#+end_src
...
#+begin_src emacs-lisp
  (digraphel-backend-vertex-as-string :value "X")
#+end_src
...
#+RESULTS:
: digraphel-backend-pair-as-string
...
#+RESULTS:
: #s(buttercup-suite "digraphel-backend-arglist" nil passed nil nil nil nil (#s(buttercup-spec ":test 'eq" #0 passed nil nil nil nil #[nil ((buttercup-with-converted-ert-signals (let ((arglist (digraphel-backend-arglist :test 'eq))) (expect (plist-get arglist :test) :to-equal 'eq)))) (t)])) nil nil nil nil)
...
#+RESULTS:
: #s(buttercup-suite "digraphel-backend-vertices" nil passed nil nil nil nil (#s(buttercup-spec "X->Y; X->Z
: W->X; W->Y" #0 passed nil nil nil nil #[nil ((condition-case err (let ((buttercup--stackframe-marker 1)) (ignore buttercup--stackframe-marker) (buttercup-expect (let ((temp127 '(sort vertices))) (let ((expr temp127)) (oclosure--fix-type (ignore expr) #'(lambda nil (:documentation 'buttercup--thunk) (if t nil expr) (sort vertices))))) :to-equal (let ((temp128 '(sort bench))) (let ((expr temp128)) (oclosure--fix-type (ignore expr) #'(lambda nil (:documentation 'buttercup--thunk) (if t nil expr) (sort bench))))))) (ert-test-failed (buttercup-fail "%S" err)) (ert-test-skipped (buttercup-skip "Skipping: %S" err)))) ((bench "W" "X" "Y" "Z") (vertices "X" "Y" "Z" "W"))])) nil nil nil nil)
...
#+RESULTS:
: #s(hash-table test equal data ("V" nil "X" ("Y" "Z") "W" ("X" "Y")))
...
*** showcase
:PROPERTIES:
:header-args: :tangle ./showcase.el
:header-args+: :emacs-lisp :lexical t
:END:
...
*** buttercup
:PROPERTIES:
:header-args: :tangle ./buttercup.el
:header-args+: :emacs-lisp :lexical t
:END:
